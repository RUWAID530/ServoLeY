...
export default function Availability() {
  ...
  const fetchWeeklyAvailability = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/provider/availability/weekly`, {
        headers: {
  ...
  const fetchSpecialAvailability = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`${API_BASE}/api/provider/availability/special`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const handleSubmitTimeSlot = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingTimeSlot 
        ? `${API_BASE}/api/provider/availability/weekly/${editingTimeSlot.id}`
        : `${API_BASE}/api/provider/availability/weekly`
  ...
  const handleSubmitSpecialDay = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = editingSpecialDay 
        ? `${API_BASE}/api/provider/availability/special/${editingSpecialDay.id}`
        : `${API_BASE}/api/provider/availability/special`
  ...
  const handleDeleteTimeSlot = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`${API_BASE}/api/provider/availability/weekly/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleDeleteSpecialDay = async (id: string) => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`${API_BASE}/api/provider/availability/special/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
